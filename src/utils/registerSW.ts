...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/recover-zip/sw.js, scope=/recover-zip/）。
    // scope がツールの全 locale ページ（/recover-zip/, /recover-zip/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/recover-zip/sw.js', {
      scope: '/recover-zip/',
    });
